...
# ===== CALCULATIONS =====
df_filtered["Profit"] = (
    (df_filtered["Unit_Price"] - df_filtered["Unit_Cost"]) *
    df_filtered["Quantity_Sold"]
)

total_revenue = df_filtered["Sales_Amount"].sum()
...
# ===== REVENUE BY REGION =====
fig_region = px.bar(
    df_filtered.groupby("Region")["Sales_Amount"].sum().reset_index(),
    x="Region",
    y="Sales_Amount",
...
# ===== SALES CHANNEL =====
fig_channel = px.bar(
    df_filtered.groupby("Sales_Channel")["Sales_Amount"].sum().reset_index(),
    x="Sales_Channel",
    y="Sales_Amount",
...
# ===== CUSTOMER TYPE =====
fig_customer = px.bar(
    df_filtered.groupby("Customer_Type")["Sales_Amount"].sum().reset_index(),
    x="Customer_Type",
    y="Sales_Amount",
...
# ===== TOP SALES REPS =====
sales_rep = df_filtered.groupby("Sales_Rep")["Sales_Amount"].sum().reset_index()

fig_sales_rep = px.bar(
    sales_rep.sort_values("Sales_Amount", ascending=False).head(10),
    x="Sales_Amount",
    y="Sales_Rep",
    orientation="h",
